...
public interface ManagerDashboardRepository extends AbstractRepository {
	...
	@Query("select count(u) from UserStory u where u.priority = acme.entities.userstories.UserStoryPriority.COULD and u.manager.id = :managerId and u.draftMode = false")
	int totalCouldUserStories(int managerId);

	@Query("select count(u) from UserStory u where u.priority = acme.entities.userstories.UserStoryPriority.SHOULD and u.manager.id = :managerId and u.draftMode = false")
	int totalShouldUserStories(int managerId);

	@Query("select count(u) from UserStory u where u.priority = acme.entities.userstories.UserStoryPriority.MUST and u.manager.id = :managerId and u.draftMode = false")
	int totalMustUserStories(int managerId);

	@Query("select count(u) from UserStory u where u.priority = acme.entities.userstories.UserStoryPriority.WONT and u.manager.id = :managerId and u.draftMode = false")
